...
export async function GET(request: NextRequest, context: Props) {
  try {
    ...
    return NextResponse.json({ 
      success: false, 
      error: 'PDF-Generierung nicht verfügbar - Submissions werden nicht mehr lokal gespeichert',
      message: 'Submissions werden nur noch per E-Mail versendet ohne lokale Speicherung'
    }, { status: 404 });
